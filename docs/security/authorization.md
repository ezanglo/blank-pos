...
- **Catalog (Phase 2+):** under **`/{businessSlug}/catalog/…`**, product, category, inventory, and price **mutations** use **`requireCatalogManager`** in [`lib/catalog-access.ts`](../../lib/catalog-access.ts) (**`owner`** \| **`manager`** only). Read paths and POS prep may use **`requireCatalogMember`** (any org **`member`**, including **`cashier`**). Do not trust client-supplied org or location IDs without server-side membership and branch checks.
