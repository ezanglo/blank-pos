...
**Status (repo):** Core Phase 3 **MVP is implemented** — register at **`/{businessSlug}/l/{locationSlug}/pos`**, [`createSale`](../../lib/actions/sales.ts), responsive cart (**`Sheet`** on small viewports, **`aside`** on **`lg+`**), sale-complete **`Dialog`**, in-app receipt preview **`Sheet`** (shared [`PosReceiptDocument`](../../components/pos/pos-receipt-document.tsx)), print rules in [`globals.css`](../../app/globals.css) (`#pos-receipt-root`), **last-receipt** shortcut from the DB + toolbar badge, cart store, category-scoped add-ons and special instructions. The checklists below match the codebase; a few items remain **optional** or **informal QA** rather than missing product features.
...
- [x] **Receipt markup (shared):** [`PosReceiptDocument`](../../components/pos/pos-receipt-document.tsx) renders branded content (`#pos-receipt-root`): logo, header/footer copy, line items with **indented add-on lines**, totals, transaction id, branch address, timestamp, cashier display name. Preview shape is built with [`transactionBundleToPreviewModel`](../../lib/pos/receipt-preview.ts) from [`getTransactionReceiptBundle`](../../lib/queries/transactions.ts).
...
- [x] **Sale complete UX:** centered **`Dialog`** (not a full-page takeover) with **Print receipt** and **New sale**; printing uses the sheet flow above.
